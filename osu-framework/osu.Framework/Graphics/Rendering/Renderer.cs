...
namespace osu.Framework.Graphics.Rendering
{
    ...
    public abstract class Renderer : IRenderer
    {
        ...
        // EMPYREAN: aggressively front-load texture uploads to VRAM. Upstream throttles uploads to
        // 32 textures / 2MB pixels per frame, which spreads a map's hit-object and skin textures
        // across many frames and causes a frame drop each time new notes/elements first appear.
        // Raising these limits uploads (almost) everything in the first few frames instead: there
        // may be a brief hitch right as a map starts, after which there are no per-note upload
        // stalls. This is the tradeoff EMPYREAN wants (a little load-time lag for steady FPS during
        // play). These remain public setters, so a host could dial them back for huge sets.
        public int MaxTexturesUploadedPerFrame { get; set; } = 8192;
        public int MaxPixelsUploadedPerFrame { get; set; } = 1024 * 1024 * 256;
        ...
        protected internal virtual void BeginFrame(Vector2 windowSize)
        {
            ...
            // EMPYREAN: drain the entire queued upload set each frame (was Count / 2) so textures
            // reach VRAM in as few frames as possible, rather than trickling in and stuttering as
            // new notes appear. The per-frame pixel ceiling below still bounds a single frame's work.
            int targetUploads = Math.Clamp(textureUploadQueue.Count, 1, MaxTexturesUploadedPerFrame);
            int uploads = 0;
