...
namespace osu.Framework.Graphics.Containers
{
    ...
    public abstract partial class CompositeDrawable : Drawable
    {
        ...
        // EMPYREAN: default masking smoothness reduced from 1px to the minimum. Every masked
        // container otherwise runs a 1px antialiasing blend band in the fragment shader each frame;
        // at the cost of crisper (aliased) edges this removes that per-pixel work app-wide, which is
        // a direct GPU saving in a GPU-bottlenecked pipeline. (0 is illegal — the shader divides by
        // this — so we use the same 0.01 floor the setter enforces.)
        private float maskingSmoothness = 0.01f;
