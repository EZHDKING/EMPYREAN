...
namespace osu.Framework.Graphics.Containers
{
        ...
        where T : Drawable
    {
        ...
        {
            // EMPYREAN: allow a subclass to substitute the shader used to draw the captured frame to
            // the screen (e.g. a sharpening pass). Falls back to the default texture shader.
            TextureShader = GetCustomTextureShader(shaders) ?? shaders.Load(VertexShaderDescriptor.TEXTURE_2, FragmentShaderDescriptor.TEXTURE);
            blurShader = shaders.Load(VertexShaderDescriptor.TEXTURE_2, FragmentShaderDescriptor.BLUR);
            grayscaleShader = shaders.Load(VertexShaderDescriptor.TEXTURE_2, FragmentShaderDescriptor.GRAYSCALE);
        }

        /// <summary>
        /// EMPYREAN hook: a subclass may return a custom shader to draw the buffered result with
        /// (for post-process effects like sharpening). Returning null uses the default.
        /// </summary>
