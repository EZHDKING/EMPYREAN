...
namespace osu.Framework.Timing
{
    ...
    public class ThrottledFrameClock : FramedClock, IDisposable
    {
        ...
        /// </remarks>
        // EMPYREAN: default raised from 10000 to MaxValue so the clock imposes no frame ceiling of
        // its own; the only limit is how fast frames actually complete.
        ...
        private double sleepAndUpdateCurrent(double milliseconds)
        {
            ...
            double startSource = SourceTime;

            // For small waits, avoid Thread.Sleep because it is coarse and can introduce stutter.
            // Use a short sleep for larger waits and spin-wait the remainder for tighter timing.
            if (milliseconds > 2.0)
            {
                double sleepMilliseconds = milliseconds - 1.0;
                TimeSpan timeSpan = TimeSpan.FromMilliseconds(sleepMilliseconds);

                if (nativeSleep?.Sleep(timeSpan) != true)
                    Thread.Sleep(timeSpan);
            }

            while (SourceTime - startSource < milliseconds)
                Thread.SpinWait(20);
