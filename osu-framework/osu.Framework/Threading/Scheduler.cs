...
namespace osu.Framework.Threading
{
    ...
    public class Scheduler
    {
        ...
        public bool AddOnce<T>(Action<T> task, T data)
        {
            lock (queueLock)
            {
                foreach (var scheduled in runQueue)
                {
                    if (scheduled is ScheduledDelegateWithData<T> existing && existing.Task == task)
                    {
                ...
                enqueueNoLock(new ScheduledDelegateWithData<T>(task, data));
            }
        ...
        public bool AddOnce(Action task)
        {
            lock (queueLock)
            {
                foreach (var scheduled in runQueue)
                {
                    if (scheduled.Task == task)
                        return false;
                }

                enqueueNoLock(new ScheduledDelegate(task));
            }

            return true;
        }

        private void addDelayedTaskNoLock(ScheduledDelegate task)
        {
            ...
            if (timedTasks.Count % LOG_EXCESSSIVE_QUEUE_LENGTH_INTERVAL == 0)
            {
                ...
                Logger.Log($"- Last task: {timedTasks[timedTasks.Count - 1]}", LoggingTarget.Performance);
            }
        }

        private void enqueueNoLock(ScheduledDelegate task)
        {
