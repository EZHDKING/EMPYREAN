...
namespace osu.Framework.Input
{
    ...
    public abstract class MouseButtonEventManager : ButtonEventManager<MouseButton>
    {
        ...
        private void handleClick(InputState state, List<Drawable>? targets)
        {
            ...
            var inputQueue = new HashSet<Drawable>(InputQueue);
            var drawables = new List<Drawable>();

            foreach (var target in targets)
            {
                if (!inputQueue.Contains(target))
                    continue;

                if (!target.IsAlive || !target.IsPresent || !target.ReceivePositionalInputAt(state.Mouse.Position))
                    continue;
        ...
        private void handleDragStart(InputState state)
        {
            ...
            var drawables = new List<Drawable>(ButtonDownInputQueue.Count);
