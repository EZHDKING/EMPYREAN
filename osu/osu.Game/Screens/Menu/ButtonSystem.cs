...
namespace osu.Game.Screens.Menu
{
    public partial class ButtonSystem : Container, IStateful<ButtonSystemState>, IKeyBindingHandler<GlobalAction>
    {
        ...
        public ButtonSystem()
        {
            ...
            {
                new MainMenuButton(ButtonSystemStrings.Settings, string.Empty, OsuIcon.Settings, new Color4(192, 192, 192, 255), (_, _) => OnSettings?.Invoke(), Key.O, Key.S)
                {
                    Padding = new MarginPadding { Right = WEDGE_WIDTH },
                },
                backButton = new MainMenuButton(ButtonSystemStrings.Back, @"back-to-top", OsuIcon.PrevCircle, new Color4(0, 0, 128, 255), (_, _) =>
                {
                    switch (State)
                    {
        ...
        {
            buttonsPlay.Add(new MainMenuButton(ButtonSystemStrings.Solo, @"button-default-select", OsuIcon.Player, new Color4(192, 192, 192, 255), (_, _) => OnSolo?.Invoke(), Key.P)
            {
                Padding = new MarginPadding { Left = WEDGE_WIDTH },
            });
            buttonsPlay.Add(new MainMenuButton(ButtonSystemStrings.Multi, @"button-default-select", OsuIcon.Online, new Color4(192, 192, 192, 255), (_, _) => State = ButtonSystemState.Multi, Key.M));
            buttonsPlay.Add(new MainMenuButton(ButtonSystemStrings.Playlists, @"button-default-select", OsuIcon.Tournament, new Color4(192, 192, 192, 255), onPlaylists, Key.L));
            buttonsPlay.Add(new DailyChallengeButton(@"button-daily-select", new Color4(192, 192, 192, 255), onDailyChallenge, Key.D));
            buttonsPlay.ForEach(b => b.VisibleState = ButtonSystemState.Play);

            buttonsMulti.Add(new MainMenuButton(ButtonSystemStrings.Lounge, @"button-default-select", FontAwesome.Solid.Couch, new Color4(192, 192, 192, 255), onMultiplayer, Key.L, Key.M)
            {
                Padding = new MarginPadding { Left = WEDGE_WIDTH }
            });
            buttonsMulti.Add(new MainMenuButton(ButtonSystemStrings.RankedPlay, @"button-daily-select", FontAwesome.Solid.Crown, new Color4(192, 192, 192, 255), onRankedPlay, Key.R));
            // disabled for now to give ranked play space.
            // buttonsMulti.Add(new MainMenuButton(ButtonSystemStrings.QuickPlay, @"button-daily-select", FontAwesome.Solid.Bolt, new Color4(192, 192, 192, 255), onQuickPlay, Key.Q));
            buttonsMulti.ForEach(b => b.VisibleState = ButtonSystemState.Multi);

            buttonsEdit.Add(new MainMenuButton(EditorStrings.BeatmapEditor.ToLower(), @"button-default-select", OsuIcon.Beatmap, new Color4(192, 192, 192, 255), (_, _) => OnEditBeatmap?.Invoke(), Key.B,
                Key.E)
            {
                Padding = new MarginPadding { Left = WEDGE_WIDTH },
            });
            buttonsEdit.Add(new MainMenuButton(SkinEditorStrings.SkinEditor.ToLower(), @"button-default-select", OsuIcon.SkinB, new Color4(192, 192, 192, 255), (_, _) => OnEditSkin?.Invoke(), Key.S));
            buttonsEdit.ForEach(b => b.VisibleState = ButtonSystemState.Edit);

            buttonsTopLevel.Add(new MainMenuButton(ButtonSystemStrings.Play, @"button-play-select", OsuIcon.Logo, new Color4(192, 192, 192, 255), (_, _) => State = ButtonSystemState.Play, Key.P, Key.M,
                Key.L)
            {
                Padding = new MarginPadding { Left = WEDGE_WIDTH },
            });
            buttonsTopLevel.Add(new MainMenuButton(ButtonSystemStrings.Edit, @"button-play-select", OsuIcon.EditCircle, new Color4(192, 192, 192, 255), (_, _) => State = ButtonSystemState.Edit, Key.E));
            buttonsTopLevel.Add(new MainMenuButton(ButtonSystemStrings.Browse, @"button-default-select", OsuIcon.Beatmap, new Color4(192, 192, 192, 255), (_, _) => OnBeatmapListing?.Invoke(), Key.B,
                Key.D));

            if (host.CanExit)
                buttonsTopLevel.Add(new MainMenuButton(ButtonSystemStrings.Exit, string.Empty, OsuIcon.CrossCircle, new Color4(0, 0, 128, 255), (_, e) => OnExit?.Invoke(e), Key.Q));
