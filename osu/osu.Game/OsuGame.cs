...
namespace osu.Game
{
    ...
    public partial class OsuGame : OsuGameBase, IKeyBindingHandler<GlobalAction>, ILocalUserPlayInfo, IPerformFromScreenRunner, IOverlayManager, ILinkHandler
    {
        ...
        // EMPYREAN: DOS-style command console overlay (PROJECT.md §6).
        private osu.Game.Empyrean.Overlays.EmpyreanTerminalOverlay empyreanTerminal;
        ...
        protected override void LoadComplete()
        {
            ...
            // EMPYREAN: the "DEVELOPER BUILD" watermark is suppressed for the themed client.
            // (Originally: if (!IsDeployedBuild) loadComponentSingleFile(devBuildBanner = new DevBuildBanner(), ScreenContainer.Add);)
            ...
            // EMPYREAN: DOS-style command console (PROJECT.md §6). The context bridge is cached
            // so the overlay (and any future tooling) can resolve the same live game hooks.
            var terminalContext = new osu.Game.Empyrean.GameTerminalContext();
            loadComponentSingleFile(terminalContext, Add);
            dependencies.CacheAs<osu.Game.Empyrean.Terminal.ITerminalContext>(terminalContext);
            loadComponentSingleFile(empyreanTerminal = new osu.Game.Empyrean.Overlays.EmpyreanTerminalOverlay(terminalContext), overlayContent.Add, true);
        ...
        public bool OnPressed(KeyBindingPressEvent<GlobalAction> e)
        {
            ...
            switch (e.Action)
            {
                ...
                case GlobalAction.ToggleEmpyreanTerminal:
                    empyreanTerminal?.ToggleVisibility();
                    return true;
